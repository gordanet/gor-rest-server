...
async def get_block(response: Response,
    ...
    """
    resp = await gord_client.request("getBlockRequest",
                                       params={
                                           "hash": blockId,
    ...
    if "block" in resp["getBlockResponse"]:
        # We found the block in gord. Just use it
        requested_block = resp["getBlockResponse"]["block"]
    else:
        if IS_SQL_DB_CONFIGURED:
            # Didn't find the block in gord. Try getting it from the DB
    ...
    # It's possible that the block from gord does not contain transactions
...
async def get_blocks(response: Response,
    ...
    the gord node. If this is not possible, the database is getting queryied as backup. In this case the response
    ...
    resp = await gord_client.request("getBlocksRequest",
                                       params={
                                           "lowHash": lowHash,
...
async def get_blocks_from_bluescore(response: Response,
                                    ...
                                    includeTransactions: bool = False):
    """
    Lists block beginning from a low hash (block id). Note that this function is running on a gord and not returning
    data from database.
    """
    response.headers["X-Data-Source"] = "Database"
